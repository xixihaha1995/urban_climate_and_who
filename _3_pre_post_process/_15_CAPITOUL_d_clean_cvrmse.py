...
comparison_file = "CAPITOUL_WithoutCooling_VCWG_Bypass.csv"
...
if os.path.exists(os.path.join(save_intermediate_path, comparison_file)):
    comparison = pd.read_csv(os.path.join(save_intermediate_path, comparison_file), index_col=0)
else:
    comparison = measurements.copy()
    prediction_columns = ['VCWG_Pres_Profile_Pa','VCWG_Direct', 'VCWG_Real_P0', 'VCWG_Real_EPW',
        'Bypass_Pres_Profile_Pa','Bypass_Direct', 'Bypass_Real_P0', 'Bypass_Real_EPW']
    ...
    vcwg_folder = f'{prediction_folder_prefix}\\b_vcwg_saving'
    vcwg_filename_prefix = 'CAPITOUL_MNP_OnlyVCWG_CVRMSE10'
    vcwg_presProf_lst_Pa, vcwg_direct_lst_C, vcwg_real_p0_lst_C, vcwg_real_epw_lst_C = \
        plt_tools.excel_to_direct_real_p0_real_epw(vcwg_filename_prefix, vcwg_folder,
    ...
    bypass_presProf_lst_Pa, bypass_direct_lst_C, bypass_real_p0_lst_C, bypass_real_epw_lst_C = \
        plt_tools.excel_to_direct_real_p0_real_epw(bypass_filename_prefix, bypass_folder,
                                                      vcwg_heights_profile, sensor_heights, target_interval,p0,
                                                        compare_start_time,compare_end_time, staPre_Pa_all,
                                                   mapped_indices=selected_prediction_idx, )

    comparison['VCWG_Pres_Profile_Pa'] = vcwg_presProf_lst_Pa[0]
    comparison['VCWG_Direct'] = vcwg_direct_lst_C[0]
    comparison['VCWG_Real_P0'] = vcwg_real_p0_lst_C[0]
    comparison['VCWG_Real_EPW'] = vcwg_real_epw_lst_C[0]
    comparison['Bypass_Pres_Profile_Pa'] = bypass_presProf_lst_Pa[0]
...
rural_mon_cvrmse = plt_tools.bias_rmse_r2(comparison['Urban_MNP_19m_5min_tdb_c'], comparison['Rural_MNP_1p2m_5min_tdb_c'],
                                           'Rural_MON')
vcwg_direct_cvrmse = plt_tools.bias_rmse_r2(comparison['Urban_MNP_19m_5min_tdb_c'], comparison['VCWG_Direct'],
                                             'VCWG_Direct')
vcwg_real_p0_cvrmse = plt_tools.bias_rmse_r2(comparison['Urban_MNP_19m_5min_tdb_c'], comparison['VCWG_Real_P0'],
                                                'VCWG_Real_P0')
vcwg_real_epw_cvrmse = plt_tools.bias_rmse_r2(comparison['Urban_MNP_19m_5min_tdb_c'], comparison['VCWG_Real_EPW'],
                                                'VCWG_Real_EPW')
...
all_df_lst = [comparison['Urban_MNP_19m_5min_tdb_c'], comparison['Rural_MNP_1p2m_5min_tdb_c'],
                    comparison['VCWG_Direct'], comparison['VCWG_Real_P0'], comparison['VCWG_Real_EPW'],
...
txt_info = [case_name, rural_mon_cvrmse, vcwg_direct_cvrmse, vcwg_real_p0_cvrmse, vcwg_real_epw_cvrmse,
            bypass_direct_cvrmse, bypass_real_p0_cvrmse, bypass_real_epw_cvrmse]
