...
class Building(object):
    ...
    def BEMCalc(self,canTemp,canHum,BEM,MeteoData,ParCalculation,simTime,Geometry_m,FractionsRoof,SWR,
        ...
        if os.path.exists(coordination.data_saving_file) and not coordination.save_path_clean:
            os.remove(coordination.data_saving_file)
        ...
        if not os.path.exists(coordination.data_saving_file):
            os.makedirs(os.path.dirname(coordination.data_saving_file), exist_ok=True)
            with open(coordination.data_saving_file, 'a') as f1:
                # prepare the header string for different sensors
                header_str = 'cur_datetime,WallshadeT,WalllitT,RoofT,canTemp,sensWaste,MeteoData.Tatm,MeteoData.Pre,'
        ...
        with open(coordination.data_saving_file, 'a') as f1:
            fmt1 = "%s," * 1 % (cur_datetime) + \
                   "%.3f," * 7 % (WallshadeT, WalllitT, RoofT, canTemp, self.sensWaste, MeteoData.Tatm, MeteoData.Pre) + \
