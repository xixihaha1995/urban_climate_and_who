...
latest_bypass_df = pd.read_csv(os.path.join(experiments_folder, latest_bypass),
                                 index_col=0, parse_dates=True,header=0)
latest_bypass_df = latest_bypass_df[compare_start_time:compare_end_time]
...
measurements = pd.read_csv(os.path.join(processed_folder, processed_file), index_col=0, parse_dates=True)
measurements = measurements[compare_start_time:compare_end_time]
df = pd.DataFrame()
df['Urban'] = measurements['Urban_DBT_C']
df['Latest_Bypass'] = latest_bypass_df['TempProf_cur[20.0]'] * \
                      (latest_bypass_df['PresProf_cur[20.0]'] / latest_bypass_df['MeteoData.Pre']) ** 0.286 - 273.15
df['VCWG'] = vcwg_df['TempProf_cur[19]'] * \
                (vcwg_df['PresProf_cur[19]'] / vcwg_df['MeteoData.Pre']) ** 0.286 - 273.15
df.to_excel(vcwg_ep_comparison, sheet_name='Urban Temperature (C)')
#3. sheet: roof
df = pd.DataFrame()
...
# df['Rural'] = vcwg_df['MeteoData.Tatm'] - 273.15
df.to_excel(vcwg_ep_comparison, sheet_name='wallSun Temperature (C)')
#2. sheet: wallShade
df = pd.DataFrame()
...
df.to_excel(vcwg_ep_comparison, sheet_name='wallShade Temperature (C)')
...
df.to_excel(vcwg_ep_comparison, sheet_name='sensWaste (W m-2)')
vcwg_ep_comparison.save()
themes = ['Urban Temperature (C)', 'Roof Temperature (C)', 'wallSun Temperature (C)', 'wallShade Temperature (C)',
            'sensWaste (W m-2)']
...
for _i, _theme in enumerate(themes):
    ...
    for _col in _df.columns:
        if "Urban" in _col:
            _axs[_i].plot(_df[_col], label=_col, color='k', linestyle='-.')
        elif "Rural" in _col:
            _axs[_i].plot(_df[_col], label=_col, color='k', linestyle='--')
        elif "Bypass" in _col:
            _axs[_i].plot(_df[_col], label=_col, color='red')
        else:
            _axs[_i].plot(_df[_col], label=_col)
        # _axs[_i].plot(_df[_col], label=_col)
    # _global_legend = 0
    _axs[_i].set_title(_theme)
    # specific legend font size
    _axs[_i].legend(fontsize=8)
